...
export function DashboardHeader({ totalHabits, completedHabits, isSyncing }: DashboardHeaderProps) {
  const { sensorStatus, isIOSLightFallback } = useSensors();

  const activeCount = [
    sensorStatus.pedometer,
  ...
  return (
    <View style={styles.container}>
      ...
      {/* iOS light-sensor notice */}
      {isIOSLightFallback && (
        <Text style={styles.notice}>
          ℹ️ LightSensor no disponible en iOS — hábito de sueño marcado manualmente
        </Text>
      )}
    </View>
  );
