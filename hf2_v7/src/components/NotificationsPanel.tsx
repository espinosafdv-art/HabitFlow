...
export function NotificationsPanel() {
  const { hasPermission, remindersEnabled, toggleReminders, isLoading, isSupported } = useNotificationsCtx();

  // Expo Go: notifications not supported — show informational card only
  if (!isSupported && !isLoading) {
    return (
      <View style={styles.card}>
        <View style={[styles.topBar, { backgroundColor: colors.warning }]} />
        <View style={styles.body}>
          <Text style={styles.sectionLabel}>🔔 Notificaciones</Text>
          <Text style={[styles.rowTitle, { marginBottom: 4 }]}>No disponible en Expo Go</Text>
          <Text style={[styles.rowSub, { lineHeight: 18 }]}>
            Las notificaciones push requieren un{" "}
            <Text style={{ color: colors.primary }}>Development Build</Text>.
            {"\n"}Usa{" "}
            <Text style={{ color: colors.primary }}>npx eas build</Text>{" "}o{" "}
            <Text style={{ color: colors.primary }}>npx expo run:android</Text>{" "}para habilitarlas.
          </Text>
        </View>
      </View>
    );
  ...
  const showScheduled = async () => {
    try {
      ...
      const list = await svc.getScheduledSummary();
      Alert.alert(
        "Notificaciones programadas",
        list.length ? list.join("\n") : "Sin notificaciones pendientes."
      );
    } catch {
      Alert.alert("No disponible", "Notificaciones no disponibles en Expo Go.");
    }
  };
